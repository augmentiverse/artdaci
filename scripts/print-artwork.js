...
const DEFAULT_SLUG = "lady-with-an-ermine";
const root = document.getElementById("dynamic-artwork-spread");

async function renderArtwork() {
const params = new URLSearchParams(location.search);
const requestedSlug = params.get("painting") || DEFAULT_SLUG;
...
root.innerHTML = `
  <section class="catalogue-plate"><p class="eyebrow"${artworkNumber ? "" : " hidden"}>${esc(artworkNumber)}</p><figure class="target-art"><img src="${esc(manifest.media.image)}" alt="${esc(title)}"/><figcaption><strong>${esc(title)}</strong><span>${esc(manifest.artist.name)} · ${esc(manifest.date)} · ${esc(locationName)}</span></figcaption></figure></section>
...
await renderArtwork().catch(() => {
  ...
  const indexUrl = lang === "fr" ? "index-fr.html" : lang === "ar" ? "index-ar.html" : "index.html";
  document.documentElement.lang = lang;
  document.documentElement.dir = lang === "ar" ? "rtl" : "ltr";
  document.title = `ARTDACI — ${copy.errorTitle}`;
  root.dataset.error = "true";
  root.innerHTML = `<section class="catalogue-text"><header><h1>${copy.errorTitle}</h1><p>${copy.errorMessage}</p></header><a class="button primary" href="${indexUrl}">${copy.back}</a></section>`;
});
